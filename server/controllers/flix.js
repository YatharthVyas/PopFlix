...
exports.postAddShow = async (req, res, next) => {
  ...
  let response= await query(`INSERT INTO shows (slot,price,weekend_price,m_id,t_id) values ("${time}","${price}","${w_price}",${m_id},1)`);   
...
exports.getFlixProfile = async (req, res, next) => {
  try
   { 
   let movies= await query(`SELECT name,m_id FROM movies WHERE release_date < CURDATE() ORDER BY release_date DESC LIMIT 15;`);   
   let mov= await  query(`SELECT * from movies where m_id IN (select m_id from shows where t_id=3);`);
  let theater_movies=filterMovieData(mov);  
    console.log(theater_movies)
      ...
      theater_movies:theater_movies
		...
		console.log(err,"ASSSSSSSSSSSSSSSSSSSSSSSS");
		/*TODO Error pg */
	 }
};
